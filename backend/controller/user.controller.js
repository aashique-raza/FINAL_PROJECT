...
const updateAccount = async (req, res, next) => {
  ...
  const sendVerifivationMail=async(req,res,next)=>{
    ...
    try {
      ...
      const token = jwt.sign({ userId: findUser._id }, process.env.VERIFICATION_SECRET_KEY, {
        expiresIn: "15m",
      });
      ...
      if(mailResult.response){
        return res.status(200).json({ success: true, mailrespons: mailResult.response, msg: 'verification link sent successfully', id:findUser._id ,token:token});
      }else{
  ...
  const verifyMail=async(req,res,next)=>{
    try {
      ...
      const tokenVerify = jwt.verify(verificationToken, process.env.VERIFICATION_SECRET_KEY);
  
      if (!tokenVerify) return next(errorHandler(404,'invalid link please try again'))
